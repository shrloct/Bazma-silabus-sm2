...
const updateById = (id, updateTodo) => {
    ...
    if (index !== -1) {
        ...
        fs.writeFileSync(checkFile, JSON.stringify(dataTodos, null, 2))
        console.log('Succescfully update todo: ' + id)
    } else {
...
const deleteById = (id) => {
    ...
    if (filterTodoId.length < dataTodos.length) {
        ...
        console.log(`ID ${id} yang Anda masukkan salah`)
    }
    rl.close()
}

module.exports = { todoQuestion, todos, storeTodo, getById, updateById, deleteById }
